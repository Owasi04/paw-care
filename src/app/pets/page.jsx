...
const Pets = () => {
  ...
  const handleAddPets = async (data) => {
    const formData = {
      ownerName: user?.name,
    ...
    try {
      ...
      if (res?.ok) {
        toast.success("Your Pets data has been added succcessfully");
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
              <div className="flex items-center justify-end gap-3">
                <Button
                  type="submit"
                  disabled={uploading}
                  className="w-full cursor-pointer bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-2xl transition mt-4 flex items-center justify-center gap-2"
                >
                  {uploading && <Loader2 size={18} className="animate-spin" />}
                  {uploading ? "Uploading image…" : "Add pet"}
                </Button>
              </div>
            </div>
          </form>
